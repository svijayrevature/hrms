...
public class BiometricLogServiceImpl implements BiometricLogService {
    ...
    @Override
    public List<UserReport> getUserLogReportBetweenTheDays(Timestamp startDate, Timestamp endDate) throws ParseException {
        List<UserReport> userReports = getBiometricLogDAO().getUserLogReportBetweenTheDays(startDate, endDate);
        for (UserReport ur :
                userReports) {
            ...
            if (timeStrings.length == typeStrings.length) {
                ...
                for (int i = 0; i < timeStrings.length; i++) {
                    ...
                    timestampLog.setTimestamp(CalendarUtils.convertStringDateToTimestampForFormat(timeStrings[i], "yyyy-MM-dd hh:mm:ss"));
